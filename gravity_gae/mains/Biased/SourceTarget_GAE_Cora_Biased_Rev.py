...
unwrapped_layers_kwargs = [
                    ...
                    {"layer":methods.Conv(hidden_dimension, output_dimension), 
                    "normalization_before_activation": None, 
                    "activation": None, 
                    "normalization_after_activation": None, 
                    "dropout_p": None, 
...
encoder = methods.GNN_FB(gnn_layers = [ methods.LayerWrapper(**unwrapped_layers_kwargs[0]), methods.LayerWrapper(**unwrapped_layers_kwargs[1])]).to(device) #
decoder = methods.LinkPropertyPredictorSourceTarget()
...
markdown_table = methods.pretty_print_link_performance_evaluation(mean_std_dict, "SourceTarget-GAE")
