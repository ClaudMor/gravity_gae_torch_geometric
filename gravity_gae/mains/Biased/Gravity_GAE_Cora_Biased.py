...
encoder = methods.GNN_FB(gnn_layers = [ methods.LayerWrapper(**unwrapped_layers_kwargs[0]), methods.LayerWrapper(**unwrapped_layers_kwargs[1])]) #
decoder = methods.LinkPropertyPredictorGravity(l = 0.001, train_l = False, CLAMP = 2)
...
for i in range(num_runs):
    methods.train(train_data, model,  methods.StandardLossWrapper(norm,BCEWithLogitsLoss(pos_weight = pos_weight))   , optimizer, device, num_epochs, lrscheduler = lrscheduler, early_stopping = False, val_data = None,  val_loss_fn =  None , patience = 50, retrain_data = train_data, use_sparse_representation = use_sparse_representation,  epoch_print_freq = 10  ) # methods.StandardLossWrapper(1.,methods.recon_loss)   methods.StandardLossWrapper(norm,BCEWithLogitsLoss(pos_weight = pos_weight))  methods.StandardLossWrapper(norm_val,BCEWithLogitsLoss(pos_weight = torch.tensor(neg_sampling_ratio_val)) )
...
markdown_table = methods.pretty_print_link_performance_evaluation(mean_std_dict, "Gravity-GAE")
